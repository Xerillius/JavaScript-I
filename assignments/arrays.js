...
console.log(`Car 33 is a ` + inventory.find(obj => { return obj.id === 33}).car_year + " " + inventory.find(obj => { return obj.id === 33}).car_make + " " + inventory.find(obj => { return obj.id === 33}).car_model );

// ==== Challenge 2 ====
// The dealer needs the information on the last car in their inventory.  What is the make and model of the last car in the inventory?  Log the make and model into the console.
let lastCar = inventory[inventory.length - 1];
console.log(`Car ` + lastCar.id + ` is a ` + lastCar.car_year + " " + lastCar.car_make + " " + lastCar.car_model);

// ==== Challenge 3 ====
// The marketing team wants the car models listed alphabetically on the website. Sort all the car model names into alphabetical order and log the results in the console
let carModels = [];
for (let id in inventory){
    carModels.push(inventory[id].car_model);
}
carModels.sort();
...
let carYears = [];
for (let id in inventory){
    carYears.push(inventory[id].car_year);
}
...
for (let id in inventory){
    if(inventory[id].car_year < 2000){
        oldCars.push(inventory[id].car_year);
    }
}
...
let BMWAndAudi = [];
for (let id in inventory) {
    if(inventory[id].car_make === 'BMW' || inventory[id].car_make === 'Audi'){
        BMWAndAudi.push(inventory[id]);
    }
}
